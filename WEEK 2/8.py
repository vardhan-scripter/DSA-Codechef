...
if __name__ == "__main__":
    ...
    for _ in range(n):
        ...
        contests.append([s,e,e-s])
    contests.sort(key = lambda x:x[2])
    vtimings = list(map(int,input().split()))
    vtimings.sort()
    wtimings = list(map(int,input().split()))
    wtimings = sorted(wtimings ,reverse = True)
    mintime = float("inf")
    for contest in contests:
        if(contest[1]-contest[0] < mintime):
            s,e = float("inf"),0
            for i in vtimings:
                if(i <= contest[0]):
                    s = i
